...
def main():
    ...
    if not matches:
        subprocess.run(["gh", "release", "create", tag, "--draft", "--target", revision,
                        "--title", f"Windows AI runtimes v{version}", "--notes",
                        f"Verified Windows modern, legacy and CPU runtimes. Source: {revision}. Build: {run['html_url']}"], check=True)
        pages = json.loads(subprocess.check_output(["gh", "api", "--paginate", "--slurp", f"repos/{repo}/releases?per_page=100"]))
        matches = [release for page in pages for release in page if release["tag_name"] == tag]
    assert len(matches) == 1
    assert matches[0]["draft"] and matches[0]["target_commitish"] == revision, "Only a matching draft can be updated"
    release_id = matches[0]["id"]

    def upload(path):
        release = api(f"releases/{release_id}")
        assert release["draft"] and release["target_commitish"] == revision, "Release changed during publication"
    ...
    for flavor in ("cpu", "legacy", "modern"):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            subprocess.run(["gh", "run", "download", run_id, "--name", stem, "--dir", str(root)], check=True)
