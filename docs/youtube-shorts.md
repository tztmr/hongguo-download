...
自动追剧的 YouTube 上传页使用 `firstEpisodeShorts` 字段，默认 false，兼容旧草稿。开启后自动执行“正片成功 → 首集 Shorts → 按配置清理”，之后由用户在 YouTube Studio 手动关联正片。从 v0.3.10 起，新首集任务使用完整首集，自动转换为 1:1 方形：按显示比例缩放并居中补黑边，不裁剪或拉伸，方形边长为源宽高的较大值（取偶数，上限 1080）。横屏上下补边，竖屏左右补边，处理旋转标记和非方形像素。仅不超过 180 秒时上传；超时跳过并提示，不截断。正片和首集分别查重与恢复，首集未完成前保留源文件，失败不重传正片。同一文件已作为 Shorts 上传时不再重复。

首集转换已接入后台媒体队列，使用现有 CPU/GPU 调度、暂停、取消、自动重试及输出验证。方形选项写入持久化请求并参与媒体去重；旧媒体请求缺少该字段时保持原画幅。普通手动 Shorts 上传仍检查最终画幅和时长。本次没有改动可见性或自动关联；相关视频需在 YouTube Studio 设置，要求高级功能权限，目标正片必须为公开或不公开列出：https://support.google.com/youtube/answer/14075157?hl=en 。

v0.3.10 验证：真实 FFmpeg 样本覆盖横屏、竖屏、方形、非方形像素、旋转标记，逐像素检查补边位置，并检查音频、时长和源文件完整性。另有 180 秒边界、手动上传校验、持久化重试及不同画幅任务去重回归检查。未执行真实 YouTube 发布。

## 2026-09-15 相关视频链路核查

自动追剧上传请求只在 `snippet.description` 添加正片地址，没有发送或保存 Related video 关联。YouTube Shorts 简介中的 URL 不可点击，也不会生成播放器中的相关视频入口；见 [官方链接说明](https://support.google.com/youtube/answer/13748639?hl=en)。目前 [videos.update 可写字段](https://developers.google.com/youtube/v3/docs/videos/update) 没有此关联字段，因此本应用不会宣称已自动关联。

后台任务新增「Shorts 关联」筛选，包含已清理完成的任务。每条首集 Shorts 展示对应正片和 Studio 详情页入口，通过系统浏览器打开，并说明高级功能、同频道与公开／不公开列出要求。完成文案明确只完成上传，关联需手动设置或核对；旧记录也直接显示提示，不依赖重新上传或恢复本地媒体。此筛选不表示已关联或待关联状态，Studio 保存结果才是实际依据。

新上传的正片按现有规则为公开或不公开列出；复用的旧正片可能仍是私享，须在 Studio 核对可见性。手动关联不会占用自动追剧组位，也不会触发重传或更改频道视频的可见性。

验证：42 项相关前端测试、95 项自动追剧 Rust 测试及前端构建通过；1920×1080 浏览器演示数据检查覆盖已完成任务筛选、对应正片和缺失地址提示。未执行真实频道关联、视频发布或 Windows 安装包测试；按要求仅推送代码，跳过 CI。
