...
自动追剧的 YouTube 上传页使用 `firstEpisodeShorts` 字段，默认 false，兼容旧草稿。开启后执行“正片成功 → 首集 Shorts → 关联正片”。从 v0.3.10 起，新首集任务使用完整首集，自动转换为 1:1 方形：按显示比例缩放并居中补黑边，不裁剪或拉伸，方形边长为源宽高的较大值（取偶数，上限 1080）。横屏上下补边，竖屏左右补边，处理旋转标记和非方形像素。仅不超过 180 秒时上传；超时跳过并提示，不截断。正片和首集分别查重与恢复，首集未完成前保留源文件，失败不重传正片。同一文件已作为 Shorts 上传时不再重复。
