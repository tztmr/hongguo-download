...
export function MediaScopeDialog({ kind, hasMergedVideo, title, modelName, episodeCount, busy = false, onSubmit, onClose }: Props) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section className="merge-dialog media-scope-dialog" role="dialog" aria-modal="true" aria-label={audio ? "分离背景音乐" : "提取字幕"}>
        <header><div><span className="title-marker" /><h2>{audio ? "分离背景音乐" : "提取字幕"}</h2></div><button type="button" className="icon-button" aria-label="关闭" disabled={busy} onClick={onClose}>×</button></header>
        ...
        <footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>取消</button><button type="button" className="primary-button" disabled={busy} onClick={() => onSubmit(scope)}>{busy ? "正在加入队列…" : audio ? "开始分离" : "开始提取"}</button></footer>
      </section>
    </div>
  );
}
