...
export type VideoBulkAction = "delete" | "private";
export function VideoBulkActionDialog({ action, videos, skippedPrivate = 0, channelId, channelTitle, commands, onDeleted, onUpdated, onClose }: {
      ...
      for (let index = 0; index < pending.length; index++) {
        ...
        try {
          if (deleting) {
            ...
            const updated = await commands.makeBlockedVideoPrivate(channelId, video.id);
            if (!alive.current) break;
            if (updated.id !== video.id) throw new Error("返回的视频不匹配，请刷新核对后重试");
            onUpdated(updated);
            if (updated.privacyStatus !== "private") throw new Error("YouTube 尚未确认设为私人，请刷新核对后重试");
          }
          if (!alive.current) break;
  ...
  return <div className="dialog-backdrop"><div ref={dialogRef} tabIndex={-1} className="yt-edit-dialog yt-delete-dialog" role={deleting ? "alertdialog" : "dialog"} aria-modal="true" aria-labelledby="yt-bulk-title" aria-describedby="yt-bulk-description" onKeyDown={(event) => {
    ...
    <header><h2 id="yt-bulk-title">{deleting ? "批量删除 YouTube 视频" : "封锁视频批量设为私人"}</h2></header>
    <div className="yt-editor-body">
      <p id="yt-bulk-description">频道：{channelTitle || channelId}。{deleting ? "将永久删除以下视频及 Shorts，无法恢复。请核对标题和 ID。" : "将以下封锁视频及 Shorts 的可见性改为私人，仅你和你指定的人可观看。此操作不会解除版权或地区限制。"}</p>
    ...
    <footer><button className="secondary-button" type="button" disabled={busy} onClick={onClose}>{attempted ? "关闭" : "取消"}</button>
      {remaining.length > 0 && <button className={deleting ? "secondary-button danger" : "primary-button compact"} type="button" disabled={busy} onClick={() => void submit()}>{busy ? `正在${verb}…` : attempted ? `重试剩余 ${remaining.length} 个` : `确认${deleting ? "永久删除" : "设为私人"} ${remaining.length} 个`}</button>}
    </footer>
  </div></div>;
}
