...
export function MediaModelsSettings({ model }: { model: UseAppSettingsResult }) {
  ...
  return (
    <section className="settings-section media-model-settings" id="settings-media">
      ...
      <MediaConcurrencyControl max={/Windows/i.test(navigator.userAgent) ? 10 : 5} value={settings.aiConcurrency} onChange={async value => { await model.update({ aiConcurrency: value }); }} />
