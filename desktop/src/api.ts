...
async function fetchDiscoveryPage(path: string): Promise<DiscoveryPage> {
  ...
  return {
    ...
    hasMore: Boolean(data.has_more && data.cell_id && data.session_id && data.plan_id && Number(data.next_offset) > 0),
...
export async function fetchDiscoveryMore(
  ...
  return {
    ...
    nextOffset: Number(data.next_offset || 0),
    hasMore: Boolean(data.has_more && Number(data.next_offset) > page.nextOffset),
