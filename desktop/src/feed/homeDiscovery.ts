...
export async function fetchHomeDiscovery(type: ContentType, api = liveApi): Promise<HomePage> {
  ...
  return type === "drama"
    ? { ...page, recommendationHasMore: page.hasMore, catalogNextPage: 1, hasMore: true }
    : page;
}

export async function fetchHomeDiscoveryMore(type: ContentType, page: HomePage, api = liveApi): Promise<HomePage> {
  if (type !== "drama" || !page.catalogNextPage) return api.fetchDiscoveryMore(type, page);
  ...
  // the website's independently paginated drama library without repeating page 1.
  const next = await api.fetchWebCategory("drama", {
