...
export function AutomationPage({ saveDir, channels = [], onOpenSettings, aiConcurrency, onAIConcurrencyChange, runtimeEnabled = isTauri() }: {
  ...
  const activeGroupCount = snapshot?.jobs.filter(job => job.status !== "completed" && job.status !== "skipped" && (!job.config || job.config.channel === snapshot.config?.channel)).length || 0;
  ...
  return <main className="auto-page">
    ...
    {runtimeEnabled && <section className="auto-runtime" aria-label="自动追剧后台状态">
      ...
      <p className="auto-runtime-meta">已保存目标：{savedChannel} · {savedPrivacy}。上次扫描：{timeLabel(snapshot?.lastScan || 0)}；下次扫描：{mode === "running" ? groupActive ? "名额已满，完成后立即补位" : timeLabel(snapshot?.nextScan || 0) : "等待启动或继续"}。</p>
      <div className="auto-batch-summary"><strong>1 组 · 最多 10 部 · 自动补位</strong><p>正在处理 {activeGroupCount} 部 · 空位 {Math.max(0, 10 - activeGroupCount)} 部 · {groupActive ? "名额已满" : "按空位继续监听"}</p><p>完成或跳过一部，立即监听并补入一部；其他任务继续处理。自动检查总时长，合并视频超过 12 小时则跳过并保留文件。失败待重试的任务保留名额。</p></div>
