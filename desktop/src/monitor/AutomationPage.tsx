...
const demoOutcomes: Record<string, { title: string; detail: string }> = {
  ...
  local: { title: "本地已下载 → 跳过该剧", detail: "命中唯一 ID 的已完成下载记录；即使成片已清理，仍保留去重标记。" },
  youtube: { title: "YouTube 已存在 → 跳过该剧", detail: "目标频道命中上传记录或原始剧名，不再下载或生成 AI 内容。" },
  pending: { title: "本地任务未完成 → 续接原任务", detail: "已在队列、下载中或失败待重试的同一 ID 不创建第二个任务。" },
...
export function AutomationPage({ saveDir, channels = [], onOpenSettings }: {
  ...
  const demoOutcome = draft.firstEpisodeShorts && ["youtube", "local"].includes(demo)
    ? { title: "正片已有记录 → 单独核对首集 Shorts", detail: "复用本地首集或继续首集任务；以频道、源唯一 ID 和成品类型分别查重。首集也已成功上传时跳过，未完成时仅续接首集，不重复上传正片。" }
    : demoOutcomes[demo];
  const stages = [
  ...
  return <main className="auto-page">
    ...
    <div className="auto-panel">
      ...
      <div className="auto-fixed"><CheckIcon size={16} /><div><strong>监听后先查重 · 重复则跳过</strong><small>检查目标 YouTube 频道和本地下载记录；优先比对源唯一 ID，缺少 ID 时用规范化原始剧名辅助匹配。AI 改写标题不改变去重身份。</small></div><span>必选步骤</span></div>
      ...
      <section className="auto-dedup-demo" aria-label="查重结果演示"><div className="auto-section-heading"><h3>查重结果演示</h3><span>演示数据 · 未读取本地或 YouTube</span></div>
        <Field label="模拟查重场景"><select value={demo} onChange={(e) => setDemo(e.target.value)}><option value="fresh">两边均无重复</option><option value="local">本地已下载</option><option value="youtube">YouTube 已存在</option><option value="pending">本地任务未完成</option><option value="error">查重请求失败</option></select></Field>
      ...
      {draft.firstEpisodeShorts && <section className="auto-path" aria-label="首集 Shorts 引流方案">
        ...
        <small>首集单独生成标题和描述，只写首集实际剧情；AI Key 不可用时使用原剧名、源简介与源封面。正片与首集分别记录上传结果，每个频道、每部剧最多一条首集 Shorts。失败只重试首集，不重复上传正片。</small>
      ...
      <div className="auto-inline-note">YouTube 与本地查重已前移至监听之后、下载之前。以原始剧名和源唯一 ID 关联目标频道记录，避免 AI 改写标题影响查重。</div>
