...
export function AutomationPage({ saveDir, channels = [], onOpenSettings, aiConcurrency, onAIConcurrencyChange, runtimeEnabled = isTauri() }: {
  ...
  return <main className="auto-page">
    ...
    {runtimeEnabled && <section className="auto-runtime" aria-label="自动追剧后台状态">
      ...
      {!!scanSummary?.at && <div className="auto-batch-summary"><strong>最近监听结果</strong><p>本轮已检查 {scanSummary.checked} 部 · 筛选排除 {scanSummary.filtered} 部 · 已有记录 {scanSummary.known} 部 · 新加入 {scanSummary.added} 部</p><p>{groupActive ? "媒体名额已满，释放空位后继续监听。" : scanSummary.more ? "还有下一页，正在继续寻找符合条件的新剧。" : scanSummary.added === 0 ? "本轮已扫描完，当前筛选下暂无可补入的新剧；按检查频率继续监听。" : "本轮已扫描完，已加入的任务继续处理；按检查频率监听后续新剧。"}</p></div>}
