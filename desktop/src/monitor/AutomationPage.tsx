...
const STORAGE_KEY = "hongguo.automation.settings-draft.v2";
const LEGACY_STORAGE_KEY = "hongguo.automation.settings-draft.v1";
const defaults = {
  ...studioDefaults,
...
function readDraft(): Draft {
  try { return normalizeDraft(JSON.parse(localStorage.getItem(STORAGE_KEY) ?? localStorage.getItem(LEGACY_STORAGE_KEY) ?? "null")); }
  catch { return { ...defaults }; }
}
...
export function AutomationPage({ saveDir, channels = [], onOpenSettings, aiConcurrency, onAIConcurrencyChange, runtimeEnabled = isTauri(), hidden = false }: {
  ...
  return <main hidden={hidden} className="auto-page">
    ...
    {runtimeEnabled && <section className="auto-runtime" aria-label="自动追剧后台状态">
      ...
      <p className="auto-runtime-meta">已保存目标：{savedChannel} · {savedPrivacy}。上次扫描：{timeLabel(snapshot?.lastScan || 0)}；下次扫描：{mode === "running" ? groupActive ? "组名额已满，完成上传与收尾后补位" : snapshot?.nextScan ? timeLabel(snapshot.nextScan) : "准备扫描补位" : "等待启动或继续"}。</p>
      <div className="auto-batch-summary"><strong>1 组 · 最多 10 部 · 自动补位</strong><p>组内任务 {activeGroupCount} 部 · 空位 {Math.max(0, 10 - activeGroupCount)} 部 · {groupActive ? "组名额已满" : "按空位继续监听"}</p><p>正片上传、首集 Shorts 和删除收尾都占用组名额，三者全部完成后才释放名额；任务跳过也会释放名额。</p><p>{groupActive ? "组名额已满，当前任务完成上传、Shorts 与删除收尾后再监听补位。" : "任务完成完整上传与收尾后，立即监听补位。"} 上传期间可继续下载、合并和分离背景音乐；CPU / GPU 按可用资源调度。合并视频超过 12 小时自动跳过并保留文件。</p></div>
      {!!scanSummary?.at && <div className="auto-batch-summary"><strong>最近监听结果</strong>{scanSummary.source && <p>{scanSummary.source}{!!scanSummary.deviceRound && ` · 设备轮次 ${scanSummary.deviceRound}`} · 第 {scanSummary.page || 1} 页</p>}<p>本轮已检查 {scanSummary.checked} 条 · 筛选排除 {scanSummary.filtered} 部 · 已有记录 {scanSummary.known} 部 · 新加入 {scanSummary.added} 部</p>{!!scanSummary.buffered && <p>已发现 {scanSummary.buffered} 部候选等待空位</p>}{scanSummary.note && <p>{scanSummary.note}</p>}<p>{groupActive ? "组名额已满，完成上传、Shorts 与清理收尾后继续监听。" : scanSummary.more ? "本轮还有来源或分页，按来源轮流寻找符合条件的新剧。" : scanSummary.added === 0 ? "本轮已扫描完，当前筛选下暂无可补入的新剧；按检查频率继续监听。" : "本轮已扫描完，已加入的任务继续处理；按检查频率监听后续新剧。"}</p></div>}
    ...
    <div className="auto-panel">
      ...
      {tab === 1 && <><div className="auto-panel-heading"><h2>准备完整成片</h2><p>每个阶段成功后才进入下一步；缺集或文件校验失败时暂停该剧。</p></div><div className="auto-fixed"><CheckIcon size={16} /><div><strong>自动下载、解密与全集合并</strong><small>按集数顺序合并，检查文件、时长和完整性。</small></div><span>必选步骤</span></div>{toggle("separate", "分离背景音乐", "生成保留人声的成片，已成功处理的文件不重复分离。")}{onAIConcurrencyChange && <MediaConcurrencyControl max={mediaConcurrencyMax} value={aiConcurrency} onChange={onAIConcurrencyChange} />}{toggle("subtitles", "提取字幕", "生成独立字幕文件，随成片上传到 YouTube。")}
